...
class Verifikasi extends CI_Controller {
    ...
    function cekstatus($id_daftar){
        $this->db->where('log_iddaftar', $id_daftar);
    ...
    function batal_verifikasi_dokumen($iddaftar,$dokumen_id){
        $cek=array('aksi'=>'Verifikasi');
        if(in_array($cek, $this->akses)){
            $data=array('status_verifikasi'=>0,'user_verifikasi'=>$this->session->userdata('username'));
            $this->verifikasi_model->verifikasi_dokumen($data,$iddaftar,$dokumen_id);
            header('Content-Type: application/json');
            echo json_encode(array("status" => TRUE,'error'=>FALSE,"message"=>"Data berhasil di verifikasi"));
        }else{
            ...
            if(!empty($dokumen)){
                ...
                if($dokumen->kode_jenis=='RJTL'){
                    if(!empty($dokumen->tgl_reg)){
                        ...
                        if(!empty($t2)){
                            if (!file_exists('report/rjtl/'.$t2[0].'/'.$t2[1] ."/" .$t2[2])) {
                                mkdir('report/rjtl/'.$t2[0].'/'.$t2[1]."/" .$t2[2], 0777, true);
                            }
                        } 
                    }
                    $data=array(
                    ...
                    $pdf = $this->m_pdf->load();
                    $pdf->WriteHTML($html);
                    $pdf->Output('report/rjtl/'.$t2[0].'/' .$t2[1] .'/' .$t2[2] .'/' .$no_urutsep .'.pdf','F');
                    //echo $content;
                    ...
                    //print_r($log);exit;
                    $status = $this->verifikasi_model->insertLog($log,$dokumen->no_sep);
                    if($status){
                        header('Content-Type: application/json');
                        echo json_encode(array("status" => TRUE,'error'=>FALSE,"message"=>"laporan Berhasil Digenerate",'log'=>$log,'nosep'=>$dokumen->no_sep));
                    }else{
                        header('Content-Type: application/json');
                        echo json_encode(array("status" => FALSE,'error'=>TRUE,"message"=>"Error Saat Insert Log", 'id'=>$status));
                    }
                    ...
                    if(empty($tgl_pulang) || $tgl_pulang=='0000-00-00'){
                        ...
                        if(!empty($t2)){
                            if (!file_exists('report/ritl/'.$t2[0].'/'.$t2[1] ."/" .$t2[2])) {
                                mkdir('report/ritl/'.$t2[0].'/'.$t2[1]."/" .$t2[2], 0777, true);
                            }
                        } 
                        //print_r($t1);exit;
                        $data=array(
                        ...
                        $pdf = $this->m_pdf->load();
                        $pdf->WriteHTML($html);
                        $pdf->Output('report/ritl/'.$t2[0].'/' .$t2[1] .'/' .$t2[2] .'/' .$no_urutsep .'.pdf','F');
                        //echo $content;
                        //Simpan Log Generator
                        $log=array(
                            'log_nosep'     => $dokumen->no_sep,
