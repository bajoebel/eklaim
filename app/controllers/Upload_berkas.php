...
class Upload_berkas extends CI_Controller {
    ...
    function form($id=""){
        ...
        if(in_array($cek, $this->akses)){
            ...
            $berkas= $this->upload_berkas_model->getUpload_berkas_by_id($id);
            if(!empty($berkas)) $kunjungan = $this->upload_berkas_model->getKunjungan($berkas->id_daftar);
            else $kunjungan=array();
            $data=array(
                'list_m_jenisberkas'=>$this->upload_berkas_model->getM_jenisberkas(),
                'list_m_berkas'=>$this->upload_berkas_model->getM_berkas(),
                'berkas'    => $berkas,
...
#region get data
    /** Mengambil data kunjungan dari simrs dengan webservise GET DATA V1 */
    /*
    ...
    function ruangan($jenis = "")
    {
        $cek = array('aksi' => "Index");
        if (in_array($cek, $this->akses)) {
            if($jenis=="RJTL") $grid = array(4,1);
            else $grid= array(2);
            $list = array(
                'status'    => true,
                'message'   => "OK",
                'data'      => $this->upload_berkas_model->getRuang($grid)
            );
        } else {
            $list = array(
                'status'    => false,
    ...
    function rjtl($start=0){
        $cek = array('aksi' => "Index");
        if (in_array($cek, $this->akses)) {
            $q = urldecode($this->input->get('key_word', TRUE));
            $start = intval($this->input->get('start'));
            $ruang = $this->input->get("ruang");
            $tgl=$this->input->get('tglreg');

            $this->dbsimrs = $this->load->database('simrs', true);
            $jns_layanan = array('RJ','GD');
            $row_count = $this->upload_berkas_model->countKunjungan($ruang, $tgl, $q, $jns_layanan);
            $limit=15;
            $list = array(
                'status'    => true,
                'message'   => "OK",
                ...
                'row_count'=> $row_count,
                'limit'     => $limit,
                'data'     => $this->upload_berkas_model->getKunjunganLimit($limit, $start, $ruang, $tgl, $q, $jns_layanan),
            );
        } else {
            $list = array(
                'status'    => false,
	...
	function save(){
        ...
        if(in_array($cek, $this->akses)){
            ...
            if(empty($cek)){
                ...
                //Insert data Pasien
                $pasien = $this->upload_berkas_model->getPasiensimrs($nomr);
                if(!empty($pasien)){
                    $this->db->insert('m_pasien', $pasien);
                }
            }
            /*if(!empty($ws)){
            ...
            if(empty($row)){
                $cek=$this->upload_berkas_model->cekBerkas($this->input->post('id_daftar'),$this->input->post('id_berkas'));
                if(empty($cek)){
                    ...
                    if($this->form_validation->run())
                    {
                        ...
                        if(!empty($_FILES['userfile'])){
                            ...
                            if(!empty($image)){
                                ...
                                $response=array('status'=>false,'error'=>true,'id_file'=>'','message'=>'Gagal Upload Data');
                            }
                        }else{
                ...
                if($this->form_validation->run())
                {
                        ...
                        if(!empty($_FILES['userfile'])){
                            ...
                            if(!empty($image)){
                                ...
                                $response=array('status'=>false,'error'=>true,'id_file'=>0,'message'=>'Gagal Upload Data');
                            }
                        }else{
                            $this->upload_berkas_model->updateUpload_berkas($data,$id_file);
    ...
    function getberkas($id_daftar){
        $cek = array('aksi' => 'Index');
        if (in_array($cek, $this->akses)) {
            $data=$this->upload_berkas_model->getBerkasdata($id_daftar);
            header('Content-Type: application/json');
            echo json_encode(array("status" => TRUE, "message" => "Ok", 'data'=>$data));
        } else {
            header('Content-Type: application/json');
    ...
    function pilih_berkas($id){
        $this->db->where('id_berkas',$id);
        $data=$this->db->get('m_berkas')->row();
        header('Content-Type: application/json');
